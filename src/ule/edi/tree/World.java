...
public class World extends AbstractBinaryTreeADT<LinkedList<Entity>> {
	...
	public void insert(String address, Entity e) {
		...
		private void insertRec(String address, Entity e) {
			
			if(travelLeft() == null) this.setLeft(new World());
			if(travelRight() == null) this.setRight(new World());
			...
			if(address.length() == 0){
				...
				if(this.content.contains(e)){
					for (Entity en : this.content) {
						...
						if(e.getType() == en.getType()){
							
							en.setCount(en.getCount() + e.getCount());
							return;
						}
					}
				}
			...
			if(content.isEmpty()) content.add(new Entity(Entity.FOREST));
			
			if(address.charAt(0) == '0') travelLeft().insertRec(address.substring(1), e);
			if(address.charAt(0) == '1') travelRight().insertRec(address.substring(1), e);
	...
	public long countEntity(int type) {
		...
		if(this.content.contains(new Entity(type))){
			
			long aux = 0;
			
			for(Entity en : this.content){
				if(en.equals(new Entity(type))){
					aux = en.getCount() + this.travelLeft().countEntity(type) + this.travelRight().countEntity(type);
				}
			}
			...
			return this.travelLeft().countEntity(type) + this.travelRight().countEntity(type);
		}
	}
